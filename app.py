...
import json
import requests
import datetime  # เพิ่มตัวนี้เข้ามาเพื่อใช้งานเรื่องเวลา
...
st.title("NexCall - ระบบแจ้งเตือนพยาบาล")
...
# 1. ตรวจสอบสถานะและข้อความจากพยาบาล
...
if st.button("ส่งข้อมูล"):
    ...
    # ส่ง Telegram (ใช้ค่าจาก secrets โดยตรง ชัวร์ที่สุดเจ้า)
    try:
        requests.post(
...
# วางโค้ดชุดนี้ไว้ใต้บรรทัดที่ 76 (ชิดซ้ายให้ตรงกับ if บรรทัดที่ 61)
st.divider() # ขีดเส้นคั่นให้สวยงาม
if st.button("🔄 อัปเดตสถานะการตอบกลับ"):
    st.rerun() # กดปุ่มนี้ปุ๊บ แอปจะโหลดใหม่และดึงข้อมูลล่าสุดทันทีเจ้า
