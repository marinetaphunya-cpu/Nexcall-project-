...
if st.button("ส่งข้อมูล"): # นี่คือบรรทัดที่ 67
        # ตั้งแต่บรรทัดนี้ลงไป ต้องเยื้องเข้าขวา 1 ช่วงให้ตรงกันหมด!
        full_msg = f"{urgency}\n🔔 แจ้งเตือนจาก {select_bed}\n👤 ผู้ป่วย: {patient_name}\n💬: {message}"
        
        # ประกาศตัวแปร (ให้เคาะเยื้องเท่ากับ full_msg)
        my_bot_token = st.secrets["TELEGRAM"]["TOKEN"]
        my_chat_id = st.secrets["TELEGRAM"]["CHAT_ID"]
        
        # คำสั่งส่งข้อมูล (ให้เคาะเยื้องเท่ากับ full_msg)
        requests.post(f"https://api.telegram.org/bot{my_bot_token}/sendMessage", params={'chat_id': my_chat_id, 'text': full_msg})
        
        # บรรทัดที่เหลือ (ให้เคาะเยื้องเท่ากับ full_msg ให้หมด!)
